  ...
  >
    ...
    <div v-if="$auth.loggedIn && $auth.user">
      ...
      <a-button
        v-if="!isVisible"
        type="dashed"
        :icon="!isRated ? 'plus' : 'edit'"
        block
        @click="!isRated ? add() : edit()"
      >
          ...
          <a-row type="flex" justify="space-between" align="middle">
            ...
            <a-col>
              ...
              <a-button
                ...
                :disabled="rating.scores === 0 || !rating.content"
    ...
    <div v-if="!$auth.loggedIn && !$auth.user" style="text-align: center">
      Vui lòng <router-link :to="{ name: 'login' }">đăng nhập</router-link> để
      đánh giá
    </div>
    ...
    <a-list class="comment-list" itemLayout="horizontal" :dataSource="ratings">
      <a-list-item slot="renderItem" slot-scope="item">
        <a-comment :author="item.author">
          ...
          <p slot="content">
            <a-rate :value="item.scores" disabled />{{ item.content }}
          </p>
          ...
          <template slot="actions" v-if="item.is_author">
            ...
            <a-popconfirm
              ...
              okText="Có"
              cancelText="Không"
              @confirm="onDeleteRating(item.id)"
            >
